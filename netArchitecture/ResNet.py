...
class ResNet(nn.Module):
    def __init__(self, block, input_channel, num_blocks, num_classes=10):
...
def ResNet18(input_channel):
    return ResNet(BasicBlock, input_channel, [2, 2, 2, 2])
...
class ResNet18_2D(nn.Module):
    def __init__(self, input_channel=1, label_height=10, label_width=256, with_aspp=True):
        ...
        self.resnet = ResNet18(input_channel)
        self.num_ftrs = 512
...
if __name__=="__main__":
    ...
    model = ResNet18_2D(input_channel=1)
    torch.cuda.set_device(3)
    model.cuda()
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # netArchitecture.to(device)
    summary(model, (1, 256, 128))
    # summary(netArchitecture, (32, 32, 32))